...
class Lift:
    ...
    def __init__(self, config: LiftConfig, lift_service, time_service: TimeService, idx: int):
        self.config = config
        self.lift_service = lift_service
        self.time_service = time_service
        self.idx = idx
        
        self._state = LiftState.IDLE
    ...
    def update(self, delta_time: float, current_time: float):
        if self._state == LiftState.MOVING:
        ...
        elif self._state == LiftState.IDLE:
            if self.next_floor:
                self._state = LiftState.CLOSE_DOOR
                self.door_toggle_start_time = current_time
    ...
    def _handle_immediate_pickup(self):
        if self._state == LiftState.CLOSE_DOOR:
            self._state = LiftState.OPEN_DOOR
            self.door_toggle_start_time = self.time_service.get_time()
        elif self._state in [LiftState.IDLE, LiftState.WAITING]:
            self._state = LiftState.WAITING
    ...
    def _find_possible_stops_indexes(self, from_floor: int, to_floor: int) -> List[Tuple[int, int]]:
        ...
        # Always allow waiting for lifts to complete all routes
    ...
    def _calc_distance_from_bottom(self, floor_num: int) -> float:
        return (self.lift_service.get_floor_height(floor_num) - 
                self.lift_service.get_floor_height(self.config.min_floor_num))
    ...
    def _move(self, distance: float):
        ...
        if abs(self.bottom - next_bottom) < abs(1.25 * distance):
            ...
            if self.next_floor.details and self.next_floor.details.get('no_leaving', 0) > 0:
                self.lift_service.handle_passenger_delivery(
    ...
    def _toggle_door(self, current_time: float):
        if self._state == LiftState.OPEN_DOOR:
            self._state = LiftState.WAITING
...
class LiftService:
    def __init__(self, floors_config: FloorsConfig, lift_configs: List[LiftConfig], 
                 time_service: TimeService, verbose: bool = False):
        self.floors = floors_config
        self.lift_configs = lift_configs
        self.time_service = time_service
    ...
    def _create_lifts(self):
        for i, config in enumerate(self.lift_configs):
            lift = Lift(config, self, self.time_service, i)
    ...
    def process_pending_calls(self, current_time: float):
        ...
        for call in calls_to_process:
            ...
            self._assign_call_to_lift(call)
            self.active_calls[call.passenger_id] = call
    
    def _assign_call_to_lift(self, call: Call):
        # Find available lifts
        available_lifts = [
            lift for lift in self.lifts
            if (lift.config.min_floor_num <= call.from_floor <= lift.config.max_floor_num and
                lift.config.min_floor_num <= call.to_floor <= lift.config.max_floor_num)
        ]
        
        if not available_lifts:
            print(f"No available lift for call from {call.from_floor} to {call.to_floor}")
            return
        ...
        for lift in available_lifts:
            route = lift.find_best_route(call.from_floor, call.to_floor)
            if route.total_time < best_time:
                best_time = route.total_time
                best_route = route
                best_lift = lift
        
        if best_lift and best_route:
            best_lift.add_route(best_route)
    ...
    def all_calls_completed(self) -> bool:
        ...
        lift_busy = any(lift.next_floor for lift in self.lifts)
        
        return not (has_pending or has_active or lift_busy)

class LiftSimulation:
    def __init__(self, floors_config: FloorsConfig, lift_configs: List[LiftConfig], 
                 calls: List[Dict], time_ratio: float = 2.5, verbose: bool = False):
        self.time_service = TimeService(time_ratio)
        self.lift_service = LiftService(floors_config, lift_configs, self.time_service, verbose=verbose)
    ...
    def run_simulation(self, max_time: float = 1000.0, verbose: bool = False) -> float:
        ...
        for call in self.calls:
            self.lift_service.add_call(call)
        ...
        while self.current_time < max_time:
            ...
            self.lift_service.process_pending_calls(self.current_time)
            
            # Update all lifts
            for lift in self.lift_service.lifts:
                lift.update(self.time_step, self.current_time)
            
            # Check if simulation is complete
            if self.lift_service.all_calls_completed():
                if verbose:
        ...
        avg_waiting_time = self.lift_service.get_average_waiting_time()
        if verbose:
            print(f"Completed calls: {len(self.lift_service.completed_calls)}")
            print(f"Average waiting time: {avg_waiting_time:.2f}s")
            
        return avg_waiting_time


def run_lift_simulation_for_lifts(num_lifts: int, verbose: bool = False) -> float:
    ...
    # Define lifts configurations
    lift_configs = [
        LiftConfig(-1, 6, 0, 8, 1.5, 6, 1.5)
    ...
    if verbose:
        print(f"Running simulation with {num_lifts} lifts and {num_calls} calls...")
    sim = LiftSimulation(floors_config, lift_configs, test_calls, verbose=verbose)
    awt = sim.run_simulation()
    return awt
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lift simulation")
    parser.add_argument('--num_lifts', type=int, required=True, help='Number of lifts to simulate')
    parser.add_argument('--verbose', action='store_true', default=False, help='Enable verbose output')
    args = parser.parse_args()

    max_num_lifts = args.num_lifts
    verbose = args.verbose

    awts = []
    for num_lifts in range(1, max_num_lifts + 1):
        if verbose:
            print(f"\nTesting with {num_lifts} lift:")
        awts.append(run_lift_simulation_for_lifts(num_lifts, verbose=verbose))
    

    coefs = []
    # Print header
    print('AWT ratios for different numbers of lifts:')
    print("The top-left cells means if the number of lifts is increased from 1 to 2, AWT is divided by this value.")
    ...
    for i, awt in enumerate(awts, start=1):
        ...
        for j in range(i + 1, max_num_lifts + 1):
            ...
            row.append(f"{coef:.1f}")
            coefs.append(coef)
        print(" ".join(row))
    
    print(f'Tight bound on |L1|/|L2| * awt(L2)/awts(L1) is:\n{min(coefs)}')
