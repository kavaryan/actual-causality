...
def get_random_adj(n, dag_method, dag_method_arg, o_func, w_min=-5, w_max=5, n_distincts=4):
    ...
    while True:
        ...
        return dict(adj=adj, u=u, v=v, o=o, o_thr=o_thr, distincts=distincts)
...
def hp_cause(adj, u, o_func, o_thr): # dict(cf_X=cf, W=dict(zip(W, w)), o=o_func(v), o_cf=o_cf)
    n = len(adj)
    V = set(range(n))
    M = np.eye(n) - adj
    v = np.linalg.inv(M) @ u
