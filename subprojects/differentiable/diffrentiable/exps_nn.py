...
class End2EndIntervention(torch.nn.Module):
    ...
    def forward(self, scm, u: torch.Tensor,
        ...
        # v_final = v_int
        scm.soft_intervention(pX, pW, self.x_prime, v_no_int)
        v_final = scm(u)
...
def hp_cause_nn(scm, u_np, o_func, o_thr, num_restarts=5, expr_name=None, device=None):
    best_res = None
    best_o_cf = float('inf')
    for i in range(num_restarts):
        scm = copy.deepcopy(scm)
        res = hp_cause_nn_do(scm, u_np, o_func, o_thr, expr_name=expr_name, device=device)
...
def hp_cause_nn_do(scm, u_np, o_func, o_thr, expr_name=None, device=None):
    # adj     = torch.as_tensor(scm, dtype=torch.float32, device=device)
    ...
    for epoch in range(epochs):
        optim.zero_grad()
        loss, _ = model(scm, u, v_no_int, tau)
    ...
    v_int, o_cf = compute_v_and_o_lower_tri(scm, u_np, cf | W_dict, o_func)
