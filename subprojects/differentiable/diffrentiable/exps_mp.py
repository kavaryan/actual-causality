...
from exps import get_random_linear_scm_and_u, hp_cause_linear
...
class RandomQuadraticOutput:
    def __init__(self, n, w_min=-5, w_max=5, seed=42):
        ...
        self.b = rng.uniform(w_min, w_max)
        print(f"RandomQuadraticOutput ({seed=}): {len(self.w)=}, {self.b=}")
...
def one_expr_linear(n, dag_method, dag_method_arg, o_func, expr_name):
    device  = "cuda" if torch.cuda.is_available() else "cpu"
    res = get_random_linear_scm_and_u(n, dag_method, dag_method_arg, o_func, device=device)
    scm = res['scm']
    u_np = res['u']
    o_thr = res['o_thr']
    # v_np = adj_res['v']
    ...
    if n <= 10:
        tic = time.time()
        a_cause_hp = next(hp_cause_linear(scm.B, u_np, o_func, o_thr))
        toc = time.time()
        delta_hp = toc - tic
    else:
    ...
    tic = time.time()
    a_cause_hp_nn = hp_cause_nn(scm, u_np, o_func, o_thr, expr_name=expr_name, device=device)
    toc = time.time()
    time_hp_nn = toc - tic
...
def run_one_expr(args):
    ...
    return one_expr_linear(n, dag_method, dag_method_arg, o_func, expr_name=expr_name)
